...
def extract_taxa(model, biom_data):
    ...
    #: Extract species weights for each community
    ...
    phylla = [p.replace('p__', '').replace('k__', '').replace('c__','').replace('o__','').replace('f__','')
              for p in phylla]
    # Make DF of the metadata
    df_phylla = pd.DataFrame(model_topic_word.transpose(),
                             index=phylla, columns=col_names)
    # df_phylla = df_phylla.groupby(df_phylla.index).sum()
    df_phylla = df_phylla.rename({'Bacteria': 'Bacteria Unknown',
                                  'Archaea': 'Archaea Unknown'})
...
def display_comm_taxa(df_phylla, words=True, graph=False):
    ...
    for i in range(n_com):
        temp = df_phylla.sort_values('Community {}'.format(i),
...
def comm_distributions(model):
    ...
    for t in range(len(model_topic_word)):
        log_hist_plot(model_topic_word[t,:], title='Community {}'.format(t),
                      xlabel='Weight of a species in a community',
